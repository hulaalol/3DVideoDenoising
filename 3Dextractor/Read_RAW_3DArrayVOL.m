...
function [data3D] = Read_RAW_3DArray(fullFileName, stInputParameters, x,y,z,bpv)
	...
	elseif(bpv == 2)
  	...
  	elseif(bpv == 4)
		%dataLengthString = '*uint32';	% You need the *, otherwise fread returns doubles.
		% Initialize a 3D data array.
		%data3D = uint32(zeros([x,y,z]));   
	
        dataLengthString = '*single';
        data3D = single(zeros([x,y,z]));
	else
	...
	sliceNumber = 1;
	for z = stInputParameters.ZStart : stInputParameters.ZEnd
